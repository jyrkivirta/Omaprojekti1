...
function App () {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        let result = await axios.get("http://localhost:4000/tuotentti/1")
        
        if (result.data.length > 0) {
          for (var i =0; i <result.data.length; i++){
            result.data[i].kysely = []
            let kysymykset = await axios.get("http://localhost:4000/tuokysymys/" + result.data[i].tentti_id)
            ...
            if (result.data[i].kysely.length > 0) {
              ...
              let vastaukset = await axios.get("http://localhost:4000/tuovaihtoehto/" + result.data[i].kysely[j].kysymys_id)
          ...
          //tässä pitäs lähettää result.data datan initialisointi funktioon
          // ja logata konsoliin tarvittaessa
        } else {
  ...
  useEffect(() => {
    const updateData = async () => {
      try {
        let result = await axios.put("http://localhost:4000/tuotentti/1", data)
      } catch (exception) {
